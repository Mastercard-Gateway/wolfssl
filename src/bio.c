...
static int wolfSSL_BIO_BASE64_read(WOLFSSL_BIO *bio, void *buf, int len) {
  ...
  if (Base64_Decode((const byte *)buf, (word32)len, (byte *)buf, &frmtSz) !=
      0) {
 ...
 */
static int wolfSSL_BIO_SSL_read(WOLFSSL_BIO *bio, void *buf, int len,
                                WOLFSSL_BIO *front) {
  int ret;
...
static int wolfSSL_BIO_MD_read(WOLFSSL_BIO *bio, void *buf, int sz) {
  if (wolfSSL_EVP_MD_CTX_type(bio->ptr.md_ctx) == WC_NID_hmac) {
    if (wolfSSL_EVP_DigestSignUpdate(bio->ptr.md_ctx, buf, (unsigned int)sz) !=
        WOLFSSL_SUCCESS) {
      return WOLFSSL_FATAL_ERROR;
    }
  } else {
...
int wolfSSL_BIO_read(WOLFSSL_BIO *bio, void *buf, int len) {
  ...
  if (front != NULL && front->infoCb != NULL) {
    ret = (int)front->infoCb(front, WOLFSSL_BIO_CB_READ, (const char *)buf, len,
                             0, 1);
    if (ret <= 0) {
      return ret;
    }
  ...
  while (bio != NULL && ret >= 0) {
...
#if defined(XREAD) && !defined(NO_WOLFSSL_DIR) && !defined(WOLFSSL_NUCLEUS) && \
    ...
    defined(USE_WOLFSSL_IO)
        ...
        if (bio->connected)
          ...
          ret = wolfIO_RecvFrom(bio->num.fd, &bio->peer_addr, (char *)buf, len,
                                0);
        }
        if (ret == WC_NO_ERR_TRACE(WOLFSSL_CBIO_ERR_WANT_READ)) {
          bio->flags |= WOLFSSL_BIO_FLAG_RETRY;
  ...
  if (front != NULL && front->infoCb != NULL) {
    ret = (int)front->infoCb(front, WOLFSSL_BIO_CB_READ | WOLFSSL_BIO_CB_RETURN,
...
static int wolfSSL_BIO_BASE64_write(WOLFSSL_BIO *bio, const void *data,
  ...
  if (bio->flags & WOLFSSL_BIO_FLAG_BASE64_NO_NL) {
    if (Base64_Encode_NoNl((const byte *)data, inLen, NULL, &sz) !=
  ...
  if (bio->flags & WOLFSSL_BIO_FLAG_BASE64_NO_NL) {
    if (Base64_Encode_NoNl((const byte *)data, inLen, tmp, &sz) < 0) {
      WOLFSSL_MSG("Base64_Encode_NoNl error");
      XFREE(tmp, heap, DYNAMIC_TYPE_TMP_BUFFER);
      return WOLFSSL_FATAL_ERROR;
    }
  } else {
    if (Base64_Encode((const byte *)data, inLen, tmp, &sz) < 0) {
 ...
 */
static int wolfSSL_BIO_SSL_write(WOLFSSL_BIO *bio, const void *data, int len,
                                 WOLFSSL_BIO *front) {
  int ret;
 ...
 */
static int wolfSSL_BIO_BIO_write(WOLFSSL_BIO *bio, const void *data, int len) {
  int sz1;
  int sz2;
  char *buf;
...
static int wolfSSL_BIO_MEMORY_write(WOLFSSL_BIO *bio, const void *data,
  ...
  if (wolfSSL_BUF_MEM_grow_ex(bio->mem_buf, bio->wrSz + len, 0) == 0) {
...
int wolfSSL_BIO_write(WOLFSSL_BIO *bio, const void *data, int len) {
  ...
  /* info cb, abort if user returns <= 0*/
  if (front != NULL && front->infoCb != NULL) {
    ret = (int)front->infoCb(front, WOLFSSL_BIO_CB_WRITE, (const char *)data,
                             len, 0, 1);
    if (ret <= 0) {
      return ret;
    }
  ...
  while (bio != NULL && ret >= 0) {
...
#ifdef WOLFSSL_BASE64_ENCODE
        ret = retB64 = wolfSSL_BIO_BASE64_write(
            bio, data, (word32)len, (byte **)&frmt, &frmtSz, front->heap);
        if (ret > 0) {
    ...
    defined(USE_WOLFSSL_IO)
        ...
        if (bio->connected)
          ...
          ret =
              wolfIO_SendTo(bio->num.fd, &bio->peer_addr, (char *)data, len, 0);
        if (ret == WC_NO_ERR_TRACE(WOLFSSL_CBIO_ERR_WANT_WRITE)) {
          bio->flags |= WOLFSSL_BIO_FLAG_RETRY;
        }
        if (ret < 0) {
...
exit_chain:
  ...
  if (front != NULL && front->infoCb != NULL) {
    ret =
        (int)front->infoCb(front, WOLFSSL_BIO_CB_WRITE | WOLFSSL_BIO_CB_RETURN,
...
int wolfSSL_BIO_gets(WOLFSSL_BIO *bio, char *buf, int sz) {
  ...
  case WOLFSSL_BIO_MD:
    if (wolfSSL_EVP_MD_CTX_size(bio->ptr.md_ctx) > sz) {
      ...
      ret = wolfSSL_EVP_DigestFinal(bio->ptr.md_ctx, (unsigned char *)buf,
                                    &szOut);
...
int wolfSSL_BIO_supports_pending(const WOLFSSL_BIO *bio) {
  while (bio) {
    if (bio->type == WOLFSSL_BIO_SSL || bio->type == WOLFSSL_BIO_MEMORY ||
...
size_t wolfSSL_BIO_ctrl_pending(WOLFSSL_BIO *bio) {
  ...
  if (bio->type == WOLFSSL_BIO_MD || bio->type == WOLFSSL_BIO_BASE64) {
...
WOLFSSL_API long wolfSSL_BIO_int_ctrl(WOLFSSL_BIO *bp, int cmd, long larg,
                                      int iarg) {
  (void)bp;
  (void)cmd;
  (void)larg;
...
int wolfSSL_BIO_set_write_buf_size(WOLFSSL_BIO *bio, long size) {
  ...
  bio->ptr.mem_buf_data =
      (byte *)XMALLOC(size, bio->heap, DYNAMIC_TYPE_OPENSSL);
  if (bio->ptr.mem_buf_data == NULL) {
...
int wolfSSL_BIO_make_bio_pair(WOLFSSL_BIO *b1, WOLFSSL_BIO *b2) {
  ...
  if (b1->ptr.mem_buf_data == NULL &&
      wolfSSL_BIO_set_write_buf_size(b1, WOLFSSL_BIO_SIZE) != WOLFSSL_SUCCESS) {
    return WOLFSSL_FAILURE;
  }

  if (b2->ptr.mem_buf_data == NULL &&
      wolfSSL_BIO_set_write_buf_size(b2, WOLFSSL_BIO_SIZE) != WOLFSSL_SUCCESS) {
    return WOLFSSL_FAILURE;
  }
...
int wolfSSL_BIO_reset(WOLFSSL_BIO *bio) {
  ...
  case WOLFSSL_BIO_MD:
    if (bio->ptr.md_ctx != NULL) {
      const WOLFSSL_EVP_MD *md = wolfSSL_EVP_MD_CTX_md(bio->ptr.md_ctx);
...
int wolfSSL_BIO_seek(WOLFSSL_BIO *bio, int ofs) {
  ...
  if (bio->type == WOLFSSL_BIO_FILE && XFSEEK(bio->ptr.fh, ofs, SEEK_SET) < 0) {
    return WOLFSSL_FATAL_ERROR;
  }
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_f_md(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_MD);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_f_buffer(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_BUFFER);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_s_bio(void) {
  static WOLFSSL_BIO_METHOD bio_meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_BIO);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_f_ssl(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_SSL);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_s_socket(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_SOCKET);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_s_datagram(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_DGRAM);
...
int wolfSSL_BIO_do_accept(WOLFSSL_BIO *b) {
  ...
  if (b->num.fd == SOCKET_INVALID) {
    ...
    wolfSSL_BIO_set_callback(new_bio, wolfSSL_BIO_get_callback(b));
    wolfSSL_BIO_set_callback_arg(new_bio, wolfSSL_BIO_get_callback_arg(b));
...
WOLFSSL_BIO *wolfSSL_BIO_new_ssl(WOLFSSL_CTX *ctx, int client) {
  ...
  if (err == 0 &&
      wolfSSL_BIO_set_ssl(sslBio, ssl, WOLFSSL_BIO_CLOSE) != WOLFSSL_SUCCESS) {
...
long wolfSSL_BIO_set_conn_hostname(WOLFSSL_BIO *b, char *name) {
  ...
  if (b->ip == NULL) {
    ...
    if (currLen != newLen) {
      b->ip =
          (char *)XREALLOC(b->ip, newLen + 1, b->heap, DYNAMIC_TYPE_OPENSSL);
      if (b->ip == NULL) {
...
#endif
{
  ...
  bio = (WOLFSSL_BIO *)XMALLOC(sizeof(WOLFSSL_BIO), 0, DYNAMIC_TYPE_OPENSSL);
  if (bio) {
    ...
    if ((bio->type == WOLFSSL_BIO_SOCKET) || (bio->type == WOLFSSL_BIO_DGRAM)) {
    ...
    if (method->type == WOLFSSL_BIO_MEMORY || method->type == WOLFSSL_BIO_BIO) {
...
int wolfSSL_BIO_free(WOLFSSL_BIO *bio) {
  ...
  if (bio) {
    ...
    if (bio->shutdown != WOLFSSL_BIO_NOCLOSE) {
      if (bio->type == WOLFSSL_BIO_MEMORY && bio->ptr.mem_buf_data != NULL) {
        if (bio->mem_buf != NULL) {
...
void wolfSSL_BIO_vfree(WOLFSSL_BIO *bio) { wolfSSL_BIO_free(bio); }
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_s_mem(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_MEMORY);
...
WOLFSSL_BIO_METHOD *wolfSSL_BIO_f_base64(void) {
  static WOLFSSL_BIO_METHOD meth = WOLFSSL_BIO_METHOD_INIT(WOLFSSL_BIO_BASE64);
...
int wolfSSL_BIO_set_ex_data_with_cleanup(
    WOLFSSL_BIO *bio, int idx, void *data,
...
/* In Visual Studio versions prior to Visual Studio 2013, the va_* symbols
  ...
  case WOLFSSL_BIO_SSL: {
    ...
    if (count >= 0) {
      pt = (char *)XMALLOC(count + 1, bio->heap, DYNAMIC_TYPE_TMP_BUFFER);
      if (pt != NULL) {
...
int wolfSSL_BIO_dump(WOLFSSL_BIO *bio, const char *buf, int length) {
  ...
  do {
    ...
    for (i = 0; i < BIO_DUMP_LINE_LEN; i++) {
      if (i < length)
        (void)XSNPRINTF(line + o, (int)sizeof(line) - o, "%02x ",
                        (unsigned char)buf[i]);
      else
